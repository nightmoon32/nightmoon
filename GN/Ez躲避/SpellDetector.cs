...
namespace ezEvade
{
    internal class SpellDetector
    {
        ...
        public SpellDetector(Menu mainMenu)
        {
            ...
            spellMenu = new Menu("敌人技能", "Spells");
                ...
                if (hero.Team != myHero.Team)
                {
                        ...
                        s => (s.charName == hero.ChampionName)))
                    {
                        ...
                        if (!onProcessSpells.ContainsKey(spell.spellName))
                        {
                            ...
                            string menuName = spell.charName + " (" + spell.spellKey.ToString() + ") 设置";

                            var enableSpell = !spell.defaultOff;

                            Menu newSpellMenu = new Menu(menuName, spell.charName + spell.spellName + "设置");
                            newSpellMenu.AddItem(new MenuItem(spell.spellName + "DodgeSpell", "躲避技能").SetValue(enableSpell));
                            newSpellMenu.AddItem(new MenuItem(spell.spellName + "DrawSpell", "显示技能").SetValue(enableSpell));
                            newSpellMenu.AddItem(new MenuItem(spell.spellName + "SpellRadius", "技能范围")
                                .SetValue(new Slider((int)spell.radius, (int)spell.radius - 100, (int)spell.radius + 100)));
                            newSpellMenu.AddItem(new MenuItem(spell.spellName + "DangerLevel", "危险等级")
