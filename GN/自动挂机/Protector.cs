...
namespace Support
{
    ...
    internal class Protector
    {
        ...
        {
            Menu = new Menu("花边-自动挂机: 保护", "Protector", true);
            ...
            skillshot.AddItem(new MenuItem("IsAboutToHitTime", "击打时间").SetValue(new Slider(200, 0, 400)));
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
            {
                skillshot.AddItem(
            ...
            // items
            var items = Menu.AddSubMenu(new Menu("物品", "Items"));
            ...
            // cc
            var cc = Menu.AddSubMenu(new Menu("控制", "CC"));
